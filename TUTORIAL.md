...
make the condition by using the filter that the person id is not equal to the current obj id, which is the condition is false as so filter returns[] empty array and find will retrun empty/false and if true find will return first instance
...
- should we update backroads app project?
  if we use the manipulating values we can use usestate
...
//if we set the fetch function seperatly / outside the useeffect and if we used the cra then es-lint will show the warning that we must include the dependency array. but if we include that we will get infinite loop
useEffect(() => {
  fetchData();
}, []);
```
...
const Example = () => {
  ...
  // this will also fail as the condition returns the value
  useEffect(() => {
    console.log('hello there');
  }, []);
...
//we can give any name to the named exports but for the defaule export we must give the exact name
...
const MultipleInputs = () => {
  //1.multiple input set one state value
  const [user, setUser] = useState({
    name: '',
    email: '',
    password: ''
  });
  //2,since we have only one state value we can use only one function
  ...
  //5.what ever the name in the obj the same name should spell correctly in the input feild of the form
  //6.if we have 15 field input then it should be 15 feild input
  ...
  //3.injecting the input value into the state by obj notation
  //4.we should give the name vlaue as we will know which value we are accessing and also we can give e.target.name
...
//for maping different name and state different name
const OtherInputs = () => {
  const [shipping, setShipping] = useState(false);
  const [framework, setFramework] = useState('react');

  //major gotcha of check box is that we have to use the checked instead of value e.target.checked
    ...
    <div>
      <form className='form'>
        ...
        <div className='form-row' style={{ textAlign: 'left' }}>
          <input type='checkbox' checked={shipping} id='shipping' name='shipping' onChange={handleShipping} />
        ...
        <div className='form-row' style={{ textAlign: 'left' }}>
          ...
          <select name='framework' id='framework' value={framework} onChange={handleFramework}>
            here we give the array value
            {frameworks.map(framework => {
              //giving the framework value as a key
...
const UncontrolledInputs = () => {
  const [value, setValue] = useState(0);
  //in form data we wont use the state direcetly focus on dom
  const handleSubmit = e => {
    e.preventDefault();
...
In React, e.currentTarget returns the DOM element that triggered the event.(<from>) also note that the dom elements means the forms,head,title,etc...

- Object From Entries

The Object.fromEntries() static method transforms a list of key-value (array of array) pairs into an object.
...
- replace it instead of use state as if we use toggle func in 15 places or more as it is reusable in many componenet
  useToggle.js
...
#### Lower State Challenge - overall concept is to push the state down so that that instead of rendering the parent component each time the child will only render so this will imporove the performance and it may not be possible in all the cases as the parent will also render along with the child so we use use memo
...
- hint addPerson fix
- while entering the single letters in the name input each time the name will reder the child component to avoid this make a seperate form componenet were the iput feild will be in a seperate component so it wont simply reender
- you can check this in react dev tools profiler
